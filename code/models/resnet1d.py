...
class ResNet(nn.Module):
    ...
    def __init__(self, depth, num_filters, block_name='BasicBlock', num_classes=10):
        ...
        self.conv1 = nn.Conv1d(64, num_filters[0], kernel_size=3, padding=1,
                               bias=False)
        ...
        self.fc = nn.Linear(num_filters[3] * 4 * block.expansion, num_classes)
    ...
    def get_bn_before_relu(self):
        if isinstance(self.layer1[0], Bottleneck):
            ...
            raise NotImplementedError('ResNet unknown chunk error !!!')
...
def resnet8x4(**kwargs):
    return ResNet(8, [32, 32, 64, 128], 'basicblock', **kwargs)
...
if __name__ == '__main__':
    ...
    x = torch.randn(1, 192, 128)
    net = resnet32x4(num_classes=512)
    ...
    flops, params = profile(net, inputs=(x, True, True))
    print(f"FLOPs: {flops / (1000 ** 2)}, Params: {params / (1000 ** 2)}")
